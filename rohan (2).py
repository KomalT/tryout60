...
F1=array([y[n-1,3],y[n-1,4],y[n-1,5]-100])
print F1
...
y=odeint(epid,yinitial,t)
F2=array([y[n-1,3],y[n-1,4],y[n-1,5]-100])
print F2
    
while (e>0.1):
    Gw=0; Gco2=0.5*Vgas; Gch4=0.5*Vgas;
    if  (e2>0.001,e3>0.001,e1>0.001):
        b1=np.multiply(a2-a1,F2)
        a3=([(a2-(np.divide(b1,(F2-F1))))])
        print a3
        t=linspace(0.0,9.0,10)
        yinitial=([0.5,0.5,0,a3[0],a3[1],a3[2]])
        y=odeint(epid,yinitial,t)
        F3=array([y[n-1,3],y[n-1,4],y[n-1,5]-100])
        ans=a2
        F1=F2
        F2=F3
        a1=a2
